...
Each Ribo-seq read is represented with its first nucleotides aligned at the P-site (Figure 1A). THe cumulative P-site counts from all reads within the selected gene range were plotted. In ggRibo plots, Ribo-seq reads are color-coded (Figure 1B): red for the first (expected) reading frame, blue for the second, and green for the third. Reads outside the ORF range are displayed in gray. RNA-seq coverage is represented with a light-yellow background.  
<img width="843" alt="image" src="https://github.com/user-attachments/assets/0fbc5088-7293-4282-9d9b-b0b42e559ef7">
...
**Additional parameters (see below): plot_range, show_seq, FASTA**
```
# show minimum uORF
ggRibo(
  gene_id = "AT3G50500",
