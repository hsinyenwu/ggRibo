...
Ribo-seq reads are shown with their first nucleotides aligned at the P-site (Figure 1A). A higher number of P-site nucleotides mapping to a nucleotide within the CDS results in a higher peak. Ribo-seq reads are color-coded (Figure 1B): red for the first (expected) reading frame, blue for the second, and green for the third. Reads outside the ORF range are displayed in gray. RNA-seq coverage is represented with a light-yellow background.
<img width="1083" alt="image" src="https://github.com/user-attachments/assets/0fbc5088-7293-4282-9d9b-b0b42e559ef7">

#### Genomic-context Ribo-seq plot provides a bird’s-eye view of the translation for all isoforms
With the single transcript style plot, it is impossible to check which transcript(s) is translated. The isoform 3 is not transcribed in the sample and leads to a confusing plot.
