...
get_RNAseq_coverage <- function(RNAseq_sample, gene_range, strand_info) {
  if (RNAseq_sample$type == "bam") {
    ...
    # Convert to coverage vector
    global_start <- start(gene_range)
    global_end <- end(gene_range)
    coverage_vec <- numeric(global_end - global_start + 1)
    if (length(cvg_gr) > 0) {
      for (i in seq_along(cvg_gr)) {
        start_pos <- max(start(cvg_gr[i]), global_start) - global_start + 1
        end_pos <- min(end(cvg_gr[i]), global_end) - global_start + 1
        if (start_pos <= end_pos) {
          coverage_vec[start_pos:end_pos] <- score(cvg_gr[i])
        }
      }
    }
    return(coverage_vec)
...
#'
#' A wrapper to construct RNAseq and Riboseq input lists from file paths.
#'
...
#' @param rna_types Character vector of file types for RNA-seq ("bam", "bigwig", "bedgraph").
#' @param ribo_types Character vector of file types for Ribo-seq ("tabular", "bigwig", "bedgraph").
...
create_seq_input <- function(rna_files = NULL, ribo_files = NULL, sample_names,
  ...
  if (include_rna && !is.null(rna_files)) {
    if (is.null(rna_types)) stop("rna_types must be provided when rna_files is specified")
  ...
  if (!is.null(ribo_files)) {
    if (is.null(ribo_types)) stop("ribo_types must be provided when ribo_files is specified")
...
plotDNAandAA <- function(GeneTxInfo, plot_range = NULL, FASTA = NULL) {
  ...
  if (any(is.na(genelim))) {
    stop(paste("Gene range is undefined for", GeneTxInfo$tx_id, 
               "- check transcript data in GRangeInfo"))
  }
...
#' Plot RNA-seq and frame-specific Ribo-seq coverage for a gene
#'
#' The `ggRibo_decom` function creates a comprehensive plot displaying RNA-seq coverage and frame-specific Ribo-seq read counts
#' for a specified gene and transcript. It includes options for displaying extended ORFs (eORFs), genomic sequences,
#' and gene models, and can scale data according to various parameters.
#'
#' @param gene_id Character string specifying the gene ID of interest.
...
#' @param selected_isoforms Optional vector of transcript IDs to plot. If provided, only these isoforms plus `tx_id` are shown.
#'
#' @return A combined ggplot object displaying RNA-seq coverage, frame-specific Ribo-seq data, gene models, and optional sequences.
#' @export
