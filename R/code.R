...
plotDNAandAA <- function(GeneTxInfo, plot_range = NULL, FASTA = NULL, nucleotide_color_scheme = "default") {
  ...
  for (frame in frames) {
    ...
    aa_seq <- suppressWarnings(as.character(translate(dna_string, genetic.code = GENETIC_CODE, if.fuzzy.codon = "X")))
...
plotDNAandAA_tx <- function(GeneTxInfo, plot_range = NULL, FASTA = NULL, nucleotide_color_scheme = "default") {
  ...
  for (frame in frames) {
    ...
    aa_str  <- as.character(translate(DNAString(dna_coding),genetic.code = GENETIC_CODE, if.fuzzy.codon="X"))
