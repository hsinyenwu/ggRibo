...
#' @param oORF_coloring Coloring scheme for overlapping ORFs: \code{"extend_mORF"} (use main CDS frame) or \code{"oORF_colors"} (use eORF-specific frames). Default is \code{"extend_mORF"}.
#'
  ...
  if (!is.null(RNAseq)) {
    for (i in seq_along(RNAseq)) {
      ...
      if (nrow(RiboRslt)>0) {
        ...
        # Identify eORF positions
        if (!is.null(eORFTxInfo)) {
          eORF_list <- eORFTxInfo$eORF_Riboseq_list[[i]]
          eORF_positions <- unique(unlist(lapply(eORF_list, function(df) df$position)))
        } else {
          eORF_positions <- integer(0)
        }

        # Determine which reads are in main ORF, eORF, or neither
        RiboRslt$in_main_orf <- RiboRslt$position %in% main_orf_positions
        RiboRslt$in_eORF <- RiboRslt$position %in% eORF_positions
        ...
        if (oORF_coloring == "extend_mORF") {
          # Extend main ORF frame across the entire transcript
          if (length(cds_tx_positions) > 0) {
            ...
            frame_df <- data.frame(position = all_positions, frame = factor(extended_frame, levels = c(0,1,2)))
            RiboRslt <- merge(RiboRslt, frame_df, by = "position", all.x = TRUE)
            # Assign frames: main ORF keeps its frame, eORF uses extended frame
            RiboRslt$plot_frame[RiboRslt$in_main_orf] <- RiboRslt$main_frame[RiboRslt$in_main_orf]
            RiboRslt$plot_frame[RiboRslt$in_eORF] <- RiboRslt$frame[RiboRslt$in_eORF]
          }
        } else if (oORF_coloring == "oORF_colors") {
          # Only oORF regions get frame colors relative to eORF start
          if (!is.null(eORFTxInfo) && length(eORF_positions) > 0) {
